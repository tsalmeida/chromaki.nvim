-- colors/chromaki-sepia-purple.lua
-- Sepia page for the main text, purple-tinted chrome & inactive windows.

local chromaki = require("chromaki")

-- Hint for FocusMode (personal.lua): soft lilac for inactive windows.
vim.g.inactive_win_bg_hint = "#e4d8f4"

chromaki.apply({
	name = "chromaki-sepia-purple",
	flavour = "latte",
	background = "light",

	-- Sepia base + shared accent palette (focus: purple UI surfaces)
	palette = {
		-- Accents (shared)
		rosewater = "#d9a2a0",
		flamingo = "#c97f6b",
	...
	custom_highlights = function(colors)
		...
		-- Purple accent for chrome elements
		local accent = colors.lavender
		local dark_accent_bg = colors.mauve
		local dark_accent_fg = colors.lavender
		...
		return {
			...
			-- Statusline: white text on a mauve strip
			...
			-- Tabline: lilac bar + coloured tabs
			...
			-- Purple border over sepia background
			FloatBorder = { fg = surface0, bg = colors.mantle },

			-- Menus (completion etc.) on lilac chrome
			...
			-- Mode messages: lavender on white
